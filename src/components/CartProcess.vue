...
provide('isPaymentFormValid', isPaymentFormValid);

const processPayment = ref(false);
provide('processPayment', processPayment);
...
function isFormDeliveryValid() {
  return nom.value.trim() !== '' && address.value.trim() !== '' && city.value.trim() !== '' && codepostale.value.trim() !== '' && (isConnected.value || email.value.trim() !== '');
}
...
<template>
  <div id="container">
    ...
    <div v-if="step === 0" style="flex: 0.7 auto; border: 1px solid black; margin: 10px; padding-left: 20px;">
      ...
      <div
        style="display: inline-flex; flex-direction: column; gap: 20px; overflow-y: auto; max-height: 60vh; padding-right: 10px; width: 98%;">
        <div v-for="product in productsInCard" :key="product.id" style="display: inline-flex; gap: 21px;">
          <div data-v-0ec6eb9a="" class="product_card" style="width: 120px; height: 180px;"
    ...
    <div v-if="step === 1" class="form-container">
      <div class="delivery-form">
        <h2 class="form-title">1. Formulaire de Livraison</h2>
        ...
        <form @submit.prevent="submitDeliveryForm">
          ...
          <div class="form-group">
            ...
            <input type="text" id="codepostale" v-model="codepostale" placeholder="Votre Code postal" required
              class="form-input" @input="formatCodePostale" />
          </div>
    ...
    <div style="flex: 0.3 1 auto; border: 1px solid black; margin: 10px; padding-left: 20px;">
      ...
      <div style="display: flex; flex-direction: column; gap: 30px; margin-bottom: 20px; padding: 0 20px;">
        ...
        <button v-if="step === 0" @click="validateCart" :disabled="productsInCard.length === 0">Valider mon
          panier</button>
        <button v-else type="submit" :disabled="!isPaymentFormValid || computeSubtotal() === 0 || !isFormDeliveryValid()"
          @click="processPayment = true">
          Payer {{ computeSubtotal ? computeSubtotal().toFixed(2).replace('.', ',') : '0,00' }} €
        </button>
